...
namespace Tests
{
    public class PCPartTests
    {
        ...
        public void Setup()
        {
            this.partName = "CPU";
            this.partCost = 100;
            this.partMultipliedCost = this.partCost * 1.2m;
            this.PartIsBroken = true;
            this.part = new PCPart(this.partName, this.partCost, this.PartIsBroken);
        }

        [Test]
        public void Constructor_ShouldInitialize_WithTwoValidParameters()
        {
            var part = new PCPart(this.partName, this.partCost);
        ...
        {
            var part = new PCPart(this.partName, this.partCost, this.PartIsBroken);
        ...
        {
            Assert.Throws<ArgumentException>(() => new PCPart(null, this.partCost));
        }

        [Test]
        public void Constructor_ShouldThrow_WithEmptyPartName()
        {
            Assert.Throws<ArgumentException>(() => new PCPart(string.Empty, this.partCost));
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new PCPart(whitespace, this.partCost));
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new PCPart(this.partName, invalidCost));
        }
